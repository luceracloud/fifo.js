...
function getFull(path, token) {
    return get(path + '?full-list=true', token);
}
            ...
            if (!worked) {
                ...
                var item1 = function() {
                    ...
                    var c = stringClient(endpoint),
                        fullEndpoint = fifoPath[endpoint] + "/oauth/token";
                    console.log('Logging into:', fullEndpoint);
                    c.post(fullEndpoint,
                    {
                        "grant_type": 'password',
                        "username": user,
                        ...
                        if (err) {
                            ...
                            if ('x-snarl-token' in res.headers) {
                                ...
                                location = fifoPath[endpoint].replace(/^(\/[^\/]+)?\/api/, '/api') + '/sessions';
                ...
                if ((user == '') || (typeof(user) == 'undefined')) {
                    var c = client(endpoint);
                    console.log('Direct API-key usage.');
...
Session = {
    list: function(token) {
        return get("/sessions", token);
    },
    ott: function(token) {
        return get("/sessions/one_time_token", token);
    }
};
...
Organization = {
    ...
    addTrigger: function(id, triggerRole, triggerPayload, token) {
        return post("/orgs/" + id + "/triggers/" + triggerRole, triggerPayload, token);
    },
...
VM = {
    ...
    metrics: function(id, token) {
        return get("/vms/" + id + "/metrics", token);
    },
    action: function(id, action, token) {
        return put("/vms/" + id + "/state", {
            "action": action
        }, token);
    },
    ...
    update: function(id, config, token) {
        ...
        // resolvers:  array of strings
        // list of VM’s resolvers

        // autoboot:   boolean
        // weather the vm boots automatically or not
        // e.q. {hostname: 'xxx', alias: 'xxx', resolvers: 'xxxx', autoboot: t/f}

        return put("/vms/" + id + "/config", config, token);
    },
    del: function(id, token) {
